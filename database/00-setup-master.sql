-- ============================================================================
-- SCRIPT MASTER: SETUP COMPLETO DO SUPABASE PARA TELA PRIME EXPRESS
...
-- Índice para VIP ativo por email (substituído por trigger)
CREATE INDEX IF NOT EXISTS idx_usuarios_vip_is_active ON usuarios_vip(email, is_active) WHERE is_active = true;
...
-- Função para desativar registros VIP antigos
CREATE OR REPLACE FUNCTION deactivate_old_vip_records()
RETURNS TRIGGER AS $$
BEGIN
  -- Desativar todos os registros VIP ativos antigos para este email
  UPDATE usuarios_vip 
  SET is_active = false, updated_at = NOW()
  WHERE email = NEW.email AND is_active = true AND id != NEW.id;
...
-- Função principal: ativar VIP após pagamento
...
BEGIN
  IF NEW.payment_status = 'approved' AND 
    ...
    INSERT INTO usuarios_vip (
      email, user_id, plano, data_inicio, data_expiraca, pagamento_aprovado, is_active,
      payment_id, payment_method, amount_paid, source
    ) VALUES (
      user_profile_record.email, NEW.user_id, vip_duration_record.plan_type,
      NOW(), new_expiry_date, true, true, NEW.payment_id, NEW.payment_method, NEW.amount, 'mercado_pago'
    );
    
    UPDATE vip_payments SET processed = true, updated_at = NOW() WHERE id = NEW.id;
...
-- Trigger para desativar registros VIP antigos
DROP TRIGGER IF EXISTS trigger_deactivate_old_vip ON usuarios_vip;
CREATE TRIGGER trigger_deactivate_old_vip
  AFTER INSERT ON usuarios_vip FOR EACH ROW 
  WHEN (NEW.is_active = true)
  EXECUTE FUNCTION deactivate_old_vip_records();
...
BEGIN
  ...
  FROM usuarios_vip WHERE email = user_email AND pagamento_aprovado = true AND is_active = true
...
BEGIN
  ...
  INSERT INTO usuarios_vip (
    email, user_id, plano, data_inicio, data_expiraca, pagamento_aprovado, is_active, source, notes
  ) VALUES (
    user_email, user_record.id,
    CASE WHEN duration_days <= 35 THEN 'mensal' WHEN duration_days <= 100 THEN 'trimestral' ELSE 'anual' END,
    NOW(), new_expiry_date, true, true, 'manual', admin_notes
  );
...
BEGIN
  ...
  SELECT COUNT(*) INTO novos_mes FROM usuarios_vip WHERE pagamento_aprovado = true AND is_active = true AND created_at >= date_trunc('month', NOW());
...
SELECT p.email, p.name, p.vip_expires_at, uv.plano, uv.data_inicio, EXTRACT(DAYS FROM p.vip_expires_at - NOW()) as dias_restantes
FROM profiles p
...
SELECT p.email, p.is_vip, p.vip_expires_at, uv.plano, uv.data_inicio, EXTRACT(DAYS FROM p.vip_expires_at - NOW()) as dias_restantes
FROM profiles p
...
INSERT INTO usuarios_vip (email, user_id, plano, data_inicio, data_expiraca, pagamento_aprovado, is_active, source, notes)
VALUES (
  'teste@telaprimeexpress.com',
  '00000000-0000-0000-0000-000000000001',
  'mensal',
...
SELECT '🎉 SETUP SUPABASE CONCLUÍDO COM SUCESSO! 🎉' as status;
